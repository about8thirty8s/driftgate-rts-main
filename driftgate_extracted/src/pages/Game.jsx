 ...
 *
 * v0.3.1 changes:
 *  - Fix async IIFE race condition (black screen on load in React Strict Mode)
 *  - cancelToken aborts async init if component unmounts before sprites load
 */
...
export default function Game({ onExit }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      // ── Load sprites ────────────────────────────────────────────────────
      const sprites = await loadSprites({}, cancelToken);
      if (cancelToken.cancelled || !sprites) return; // unmounted before load finished
