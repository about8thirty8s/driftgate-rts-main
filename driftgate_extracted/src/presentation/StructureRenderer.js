...
export class StructureRenderer {
  constructor(ctx, camera, sprites = new Map()) {
    ...
    this.camera  = camera;
    this._tick   = 0;
    this.sprites = sprites; // Map<spriteId, HTMLImageElement> from SpriteLoader
  ...
  _drawStructure(structure) {
    ...
    if (spriteImg) {
      ...
      return; // sprite handled — skip placeholder geometry
    }
  ...
  // ── Sprite structure renderer ────────────────────────────────────────────
  // Used when a sprite image is available for a structure.
  // Falls back to placeholder automatically if sprite is missing.
