...
<body>
  ...
  <script>
    // Rewrite /sprites/ paths → GitHub raw so sprites load without a local server
    const SPRITE_BASE = "https://raw.githubusercontent.com/about8thirty8s/driftgate-rts-main/main/public";
    const _NativeImage = window.Image;
    class PatchedImage extends _NativeImage {
      set src(val) {
        if (val && typeof val === 'string' && val.startsWith('/sprites/')) {
          super.src = SPRITE_BASE + val;
        } else {
          super.src = val;
        }
      }
  ...
  <script type="module" src="https://base44.app/api/apps/6a052e31974cfce1b2882157/files/mp/public/6a052e31974cfce1b2882157/dcebc0fa0_driftgate_game.js" crossorigin="anonymous"></script>
</body>
</html>`;
